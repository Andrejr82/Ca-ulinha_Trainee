...
const modeIcons: Record<GenerationMode, React.ReactNode> = {
  ...
  [GenerationMode.D3_TO_VIDEO]: <CubeIcon className="w-5 h-5" />,
...
const PromptForm: React.FC<PromptFormProps> = ({
  ...
  useEffect(() => {
    if (initialValues) {
      ...
      setModelFile(null); // Clear 3D model on new initial values
    }
  }, [initialValues]);
  ...
  useEffect(() => {
    if (
      ...
      generationMode === GenerationMode.D3_TO_VIDEO
    ) {
    ...
    canvas.toBlob(
      async (blob) => {
        if (blob) {
          ...
          try {
            const imageFile = await fileToImageFile(file);
            setReferenceImages((imgs) => [...imgs, imageFile]);
          } catch (error) {
            console.error('Error creating image file from capture:', error);
          }
        }
      },
      'image/png',
  ...
  const handleModelFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      setModelFile(file);
      // Clear previous captures when a new model is uploaded
      setReferenceImages([]);
    }
  };
  ...
  const promptPlaceholder = {
    ...
    [GenerationMode.D3_TO_VIDEO]:
      'Describe a video using the captured 3D model frames...',
  ...
  const renderMediaUploads = () => {
    ...
    if (generationMode === GenerationMode.D3_TO_VIDEO) {
      return (
        <div className="mb-3 p-4 bg-[#2c2c2e] rounded-xl border border-gray-700 flex flex-col items-center justify-center gap-4">
          {!modelFile ? (
            <button
              ...
              <span className="text-sm mt-2 px-2">Upload 3D Model</span>
              <input
                type="file"
                ref={modelInputRef}
            ...
            <>
              <ModelViewer modelUrl={modelUrl} canvasRef={canvasRef} />
              <div className="flex items-center gap-4">
                <button
                  type="button"
                  onClick={handleCaptureFrame}
                  disabled={referenceImages.length >= 3}
                  className="px-4 py-2 bg-indigo-600 rounded-lg hover:bg-indigo-700 transition-colors text-sm font-medium disabled:bg-gray-600 disabled:cursor-not-allowed">
                  Capture Frame ({referenceImages.length}/3)
                </button>
                <button
                  type="button"
                  onClick={() => setModelFile(null)}
                  className="px-4 py-2 bg-gray-600 rounded-lg hover:bg-gray-500 transition-colors text-sm font-medium">
                  Change Model
                </button>
              </div>
            </>
          ...
          {referenceImages.length > 0 && (
            <div className="mt-4 w-full">
              <p className="text-sm text-gray-400 mb-2 text-center">
                Captured Frames (used as references):
              </p>
  ...
  const submitValidation = useMemo(() => {
    switch (generationMode) {
      case GenerationMode.TEXT_TO_VIDEO:
        if (!prompt.trim()) {
          return {disabled: true, tooltip: 'Please enter a prompt.'};
        }
        break;
      case GenerationMode.FRAMES_TO_VIDEO:
        if (!startFrame) {
          return {disabled: true, tooltip: 'A start frame is required.'};
        }
        break;
      case GenerationMode.REFERENCES_TO_VIDEO:
      case GenerationMode.D3_TO_VIDEO: {
        const hasNoRefs = referenceImages.length === 0;
        const hasNoPrompt = !prompt.trim();
        if (hasNoRefs || hasNoPrompt) {
          let tooltip = 'Please add/capture reference image(s) and enter a prompt.';
          if (hasNoRefs && !hasNoPrompt) {
            tooltip = 'At least one reference image is required.';
          } else if (!hasNoRefs && hasNoPrompt) {
  ...
  }, [generationMode, prompt, startFrame, referenceImages, inputVideoObject]);

  const {disabled: isSubmitDisabled, tooltip: tooltipText} = submitValidation;
